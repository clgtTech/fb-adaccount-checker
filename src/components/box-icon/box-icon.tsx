...
import styles from './box-icon.module.scss';

export type IconHtmlAttrs = React.ComponentPropsWithoutRef<'svg'>;

export interface BoxIconProps extends IconHtmlAttrs {
  icon: 'copy' | 'facebook-circle' | 'history';
  linearGradient?: [
...
export function BoxIcon({
  ...
  const gradientId = React.useRef(uniqueId('box-icon-gradient-'));
  ...
  if (linearGradient) {
    ...
    defs = (
      <defs>
        <linearGradient id={gradientId.current} x1={x1} x2={x2} y1={y1} y2={y2}>
...
function renderPath(icon: BoxIconProps['icon']): JSX.Element {
  switch (icon) {
    case 'copy':
      return (
